...
const uploadFile = require("../middlewares/uploadExcludeStockiest");

const db = require("../models");
const ExcludeStockiest = db.stockiest;

exports.importExcludeStockiest = async (req, res) => {
    try {
      // To upload file
...
let convertExcelToJson  = (fileName) => {
  return new Promise(resolve => {
    const filePath = './public/uploads/exclude_stockiest/' + fileName;
    if (!filePath) {
    ...
    if (tempData.length <= 100100) {
      ...
      const newArray = parsedData.map(item => {
        return { 
          ...
          organization: item.Organization.toLowerCase(),
      ...
      (async function(){
        const removeAll = await ExcludeStockiest.deleteMany({});   // Remove all documents before insert.
        
        const insertMany = await ExcludeStockiest.insertMany(newArray);
