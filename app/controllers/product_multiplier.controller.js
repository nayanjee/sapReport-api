...
const uploadFile = require("../middlewares/uploadMultiplier");

const db = require("../models");
const ProductMultiplier = db.product_multiplier;

exports.importProductMultiplier = async (req, res) => {
    try {
      // To upload file
...
let convertExcelToJson  = (fileName) => {
  return new Promise(resolve => {
    const filePath = './public/uploads/multiplier/' + fileName;
    if (!filePath) {
    ...
    if (tempData.length <= 100100) {
      ...
      (async function(){
        const removeAll = await ProductMultiplier.deleteMany({});   // Remove all documents before insert.
        
        const insertMany = await ProductMultiplier.insertMany(newArray);
